...
class TextDetector(
    ...
    companion object {
        private const val LIMIT_SIDE_LEN = 1536
        private const val THRESH = 0.25f
        private const val BOX_THRESH = 0.42f
        private const val UNCLIP_RATIO = 1.85f
    ...
    private fun sortBoxes(boxes: List<TextBox>): List<TextBox> {
        ...
        val sortedByTop = boxes.sortedBy { it.points.minOf { p -> p.y } }
        val lines = mutableListOf<MutableList<TextBox>>()

        for (box in sortedByTop) {
            val boxTop = box.points.minOf { it.y }
            val boxBottom = box.points.maxOf { it.y }
            val boxHeight = max(boxBottom - boxTop, 10f)

            val matchingLine = lines.find { line ->
                val lineTop = line.minOf { b -> b.points.minOf { it.y } }
                val lineBottom = line.maxOf { b -> b.points.maxOf { it.y } }
                val overlap = max(0f, min(boxBottom, lineBottom) - max(boxTop, lineTop))
                val minH = min(boxHeight, max(lineBottom - lineTop, 10f))
        ...
        lines.sortBy { line -> line.minOf { b -> b.points.minOf { it.y } } }
        return lines.flatMap { line -> line.sortedBy { b -> b.points.minOf { it.x } } }
    }
